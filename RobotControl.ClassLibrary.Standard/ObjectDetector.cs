...
namespace RobotControl.ClassLibrary
{
    ...
    public class ObjectDetector : RobotControlBase, IObjectDetector
    {
        ...
        //private static Dictionary<string, int> bitmatFileCounters = new Dictionary<string, int>();
        //private static void SaveBitmap(Bitmap bitmap, string name)
        //{
        //    if (!bitmatFileCounters.ContainsKey(name)) bitmatFileCounters.Add(name, 0);
        //    bitmatFileCounters[name] = (bitmatFileCounters[name] + 1) % 16;
        //    var filename = $"{name}{bitmatFileCounters[name].ToString("00")}.bmp";
        ...
        private static void DisplayDetectedObjects(Bitmap bitmap, BoundingBox box)
        {
            ...
            //SaveBitmap(bitmap, "WithDetectedSquares");
        }
